...
# 
# Here using Bray-Curtis distance from 1000x iterated rarefaction
# 
##########################################################################
...
dist1000_bray <- readRDS("./out/RDS_files/dist1000_bray.rds")
...
dist1000_bray_df <- as.data.frame(as.matrix(dist1000_bray))

# Make tibble (tidy object)
dist1000_bray_tbl <- dist1000_bray_df %>% as_tibble(rownames = "sample_id_L")

# Pivot longer (so to have 3 cols)
dist1000_bray_long <- dist1000_bray_tbl %>% 
  pivot_longer(
    -sample_id_L,
...
dist1000_bray_paired <- inner_join(dist1000_bray_long, siL, by = "sample_id_L") %>% 
...
length(dist1000_bray_paired$pairwise_distance) # 729
...
# as state x colony
dist1000_bray_paired_grouped <- dist1000_bray_paired %>% 
  mutate(
...
pwcBC1000_n3_tibble <- dist1000_bray_paired_grouped %>%
  filter(state_colony %in% n3)

saveRDS(pwcBC1000_n3_tibble, "./out/RDS_files/pwcBC1000_n3_tibble.rds")



## Test difference between groups (only groups with n = 3) -------------------------------------

symb <- dist1000_bray_paired_grouped %>% 
  filter(state_colony %in% n3) %>% 
  filter(state == "S")

blea <- dist1000_bray_paired_grouped %>% 
  filter(state_colony %in% n3) %>% 
  filter(state == "B")

### Normality check ---------
# (want p-value > 0.05)
shapiro.test(symb$pairwise_distance) # p-value = 0.4106  OK
shapiro.test(blea$pairwise_distance) # p-value = 0.02751 NOT OK
...
w_n3 <- dist1000_bray_paired_grouped %>% 
  filter(state_colony %in% n3) %>% 
...
saveRDS(w_n3, "./out/RDS_files/rar1000_w_n3.rds")


## Plot by group and only groups with n = 3 - Narrow -------------------------------------
(
  pwcBC1000_n3 <- dist1000_bray_paired_grouped %>%
    filter(state_colony %in% n3) %>% 
    ggplot(., aes(
...
ggsave("./out/Gfas_16S/supplementary/bray1000_pairwisedist_bygroup_n3_narrow.png", 
       dpi = 330, 
       bg = "white",
       units = "cm", width = 6, height = 10)

saveRDS(pwcBC1000_n3, "./out/RDS_files/pwcBC1000_n3_ggplot.rds")
...
# Normality check
# (want p-value > 0.05)
shapiro.test(symb$pairwise_distance) # p-value = 0.5424  OK
shapiro.test(blea$pairwise_distance) # p-value = 0.07207 OK

# since data is normally distributed can use F-test (or also Levene ...)

# Homogeneity of variance check
# (F-test, want p-value > 0.05)
var.test(pairwise_distance ~ state, data = dist1000_bray_paired_grouped) # p-value = 0.3187 OK
...
tt_n1n3$p.value # 0.0004999743

tt_n1n3[1:10] %>% as.data.frame() 


## Plot by group all (also n = 1) ------------------------------------------------
dist1000_bray_paired_grouped %>% 
  # filter(state_colony %in% n3) %>% 
  ggplot(., aes(
    x = forcats::fct_rev(state), 
    y = pairwise_distance,
  ...
  scale_x_discrete(breaks = c("S", "B"), labels = c("Symbiotic", "Bleached")) +
  ylab("Bray-Curtis pairwise distance") +
  labs(caption = paste0(tt_n1n3$method, ", **_P_** = **",
...
ggsave("./out/Gfas_16S/supplementary/bray1000_pairwisedist_bygroup_n1n3_narrow.png", 
       dpi = 330, 
       bg = "white",
